...
static gboolean
gst_x_image_sink_ximage_put (GstXImageSink * ximagesink, GstBuffer * ximage,GstVpuDecMeta *meta) 
{
  GstXImageMemory *mem;
  ...
  if (!ximage) {
    ...
    if (ximagesink->cur_image) {
      ...
      meta = gst_buffer_get_vpudec_meta (ximage);
      if(!meta) {
...
gst_x_image_sink_xcontext_get (GstXImageSink * ximagesink)
{
  ...
  xcontext->caps = gst_caps_new_simple ("video/x-raw",
      "format", G_TYPE_STRING, "NV12",//gst_video_format_to_string (vformat),
...
gst_x_image_sink_show_frame (GstVideoSink * vsink, GstBuffer * buf)
{
  ...
  if (meta) {
	  GST_DEBUG_OBJECT(ximagesink,"get meta");
	  ;// buffer = buf;
  } else {
	  GST_ERROR_OBJECT(ximagesink,"buffer is not valid");
	  return GST_FLOW_ERROR;
  ...
  if (!gst_x_image_sink_ximage_put (ximagesink, to_put,meta))
    goto no_window;
...
gst_x_image_sink_class_init (GstXImageSinkClass * klass)
{
   ...
   */
  g_object_class_install_property(gobject_class,PROP_DRIVER_NAME,g_param_spec_string ("driver-name",
   ...
   */
  g_object_class_install_property(gobject_class,PROP_CONNECTOR_ID,g_param_spec_int ("connector-id",
   ...
   * CRTC is tried.
   */
  g_object_class_install_property(gobject_class,PROP_PLANE_ID,g_param_spec_int ("plane-id",
